...
#define  sqr(x)  ((x) * (x))
//we know n = 500 for our program
//but cannot use variable size initialization 
//int  K = floor((5/3) * cbrt(500));                       /* number of histogram bins   */
//so I calulated the real value in CaculateServiceTimes
#define K  13


  int main(void)
{
  //chose the min and max to fit all our data (none of it really seemed to be an outlier)
  double  MIN   =  0.0;
  double  MAX   =  16.0;

  double  DELTA  =  ((MAX - MIN) / K);         /* histogram bin size (width) */
